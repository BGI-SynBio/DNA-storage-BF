...
class ErrorCounterOneFile(object):
    ...
    def __init__(self, bloom_filter, expected_rate, payload_length, digital_count, avg_yield, stdev, read_depth,
                 ...
                 filter_size=0, hash_size=0, counting_type=False, record_coverage=False, verbose=False):
        """
    ...
    def run(self):
        ...
        if self.training_dnas != self.checking_dnas:
        ...
        if self.counting_type:
            ...
            self.false_positive_lst = self.filter.find_false_positive(trained_dnas, list(self.right_check.keys())
                                                                      + list(self.wrong_check.keys()),
                                                                      self.checked_keys,
                                                                      deleted_list=self.deleted_list,
                                                                      verbose=self.verbose)
    ...
    def show_results(self):
        ...
        limit_decoded_rate = str(round(self.limit_decoded_count / self.digital_count * 100, 2)) + "%"
        false_positive_rate_based_on_count = str(round(self.error_count / self.unexpected_count * 100, 6)) + "%"
        face_unexpected_rate = str(round(self.unexpected_count / self.read_count * 100, 6)) + "%"
        ...
        c_dna_count = list(self.right_check.values())
        ...
        # plt.hist(c_dna_count, bins=40, facecolor='blue', edgecolor='black', alpha=0.7)
        # plt.xlabel('count number')
        # plt.ylabel('frequency')
        # plt.title("frequency distribution histogram")
        # plt.savefig(self.file + "_" + str(self.filter._hash_function_type[0]) + "_fs" + str(self.filter._filter_size)
        #             + "_hs" + str(self.filter._hash_size) + "_rd" + str(self.read_depth) + ".jpg")
        # # plt.show()
        # plt.close()

        # print("decoded count = " + str(self.decoded_count) + ", limit decoded count = "
        #       + str(self.limit_decoded_count) + ", digital count = " + str(self.digital_count) + ".\n"
        #       + "error count = " + str(self.error_count) + ", unexpected count = " + str(self.unexpected_count)
        #       + ".\n", flush=True)
        #
        # print("decoded rate = " + decoded_rate + ", limit decoded rate = " + limit_decoded_rate + ".\n"
        #       + ", false positive rate (based on count) = " + false_positive_rate_based_on_count
        #       + ", false positive rate (based on type) = " + false_positive_rate_based_on_type + ".\n"
        #       + "face unexpected rate = " + face_unexpected_rate + ", read count = " + str(self.read_count) + ".\n"
        #       + "maximum count in right = " + str(self.max_right) + ", minimum count in right = " + str(self.min_right)
        #       + ", maximum count in wrong = " + str(self.max_wrong) + ", minimum count in wrong = "
        #       + str(self.min_wrong), flush=True)
        ...
        with open(self.file + "_" + str(self.filter._hash_function_type[0]) + "_fs" + str(self.filter._filter_size)
            ...
            file.write("filter size" + "," + str(self.filter._filter_size) + "\n"
                       ...
                       + "time for finding false positive" + "," + str(self.run_time[6]) + "min\n"

                       + "total time" + "," + str(self.run_time[7]) + "min")
