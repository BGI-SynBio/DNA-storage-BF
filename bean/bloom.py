...
class Filter(object):
    ...
    def train(self, fragments, counting_type=False, verbose=False):
        """
        ...
        for index, fragment in enumerate(fragments):
            if verbose:
                print("\rdo = " + str(index + 1) + ", total = " + str(self._data_count) + ".", end="", flush=True)
    ...
    def find_false_positive(self, trained_dnas, passed_dnas, checked_dna_keys, deleted_list=None, verbose=False):
        """
        Find out all the false positive DNA fragments from the Counting Bloom Filter.

        :param trained_dnas: dna fragments for training the Bloom Filter.
        :type: list

        :param passed_dnas: dna fragments have been detected by the Bloom Filter successfully.
        :type: list
        ...
        if verbose:
            print("Perform a deletion operation  for the Counting Bloom Filter using DNAs that only be trained."
                  , flush=True)

        false_positives_keys = np.where(np.array(self.deleted_filter_list) < 0)[0]
        missed_dnas = list(set(trained_dnas) - (set(passed_dnas) & set(trained_dnas)))
        for index, dna in enumerate(missed_dnas):
            for hash_index in range(self._hash_size):
                key = hasher.function(self._hash_function_type[hash_index % len(self._hash_function_type)], dna,
                                      hash_index) % self._filter_size
                if deleted_list:
                    deleted_list[key] -= 1
                else:
                    self.deleted_filter_list[key] -= 1
            if verbose:
                print("\rdo = " + str(index + 1) + ", total = " + str(len(missed_dnas)) + ".", end="", flush=True)
